 ...
 * with the dependency.
 * 
 * Notes: evict and respond are convenience methods.
 */
export class Stage {
  public readonly next: Stage[] = []
  constructor(
   ...
   * rejected. Other requests in the queue may be evicted to make room for 
   * this request.
   ...
   */
  public respond(request: Request, response: Response): Promise<void> { }


  /**
   * Non-overridable.
   * 
   * @param stage The next stage(s) after this one.
   */
  public addNext(stage: Stage | Stage[]): void {
    if (Array.isArray(stage)) {
      this.next.push(...stage);
    } else {
      this.next.push(stage);
    }
  }
...
class Cache extends Stage {
  constructor() {
    super(new ArrQueue(10, 10))
  }
};
class LiveService extends Stage {
  constructor() {
    super(new ArrQueue(10, 10));
  }
}
